...
void rt_OneStep(void)
{
  ...
  currentTime = (extmodeSimulationTime_T) PruebaSensores_M->Timing.clockTick0;
  PruebaSensores_step();
...
int main(void)
{
  ...
  init();
  MW_Arduino_Init();
  rtmSetErrorStatus(PruebaSensores_M, 0);
  ...
  PruebaSensores_initialize();
  cli();
  sei();

  /* External Mode initialization */
  errorCode = extmodeInit(PruebaSensores_M->extModeInfo, &rtmGetTFinal
    (PruebaSensores_M));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(PruebaSensores_M, true);
    }
  }
  ...
  runModel = !extmodeSimulationComplete() && !extmodeStopRequested() &&
    !rtmGetStopRequested(PruebaSensores_M);
